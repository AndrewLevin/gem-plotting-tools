...
def anaUltraScurve(args, scurveFilename, calFile=None, GEBtype="short", outputDir=None, vfatList=None):
    ...
    if ((vfatList is not None) and ((min(vfatList) < 0) or (max(vfatList) > 23))):
        raise ValueError("anaUltraScurve(): Either vfatList=None or entries in vfatList must be in [0,23]")
    ...
    if performFit:
        ...
        canvasBoxPlot_Thresh.SaveAs("%s/h2ScurveMeanDist_All.png"%(outputDir))
